...
import { transformArticleMeta, transformArticleContent } from './transformations'
import firebaseRefs from '../config/firebase-refs'
import getArticles from './get-articles'

export function observeArticlesMetaList (fn) {
  const requestData = fn(getArticles('home'))
...
export function observeArticleContent (id, fn) {
  const requestData = (data) => fn(transformArticleContent(data.val()))
